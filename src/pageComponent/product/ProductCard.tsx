...
const ProductCard = ({ product }: { product: IProduct }) => {
    ...
    return (
        <div className="bg-white text-gray-800 border border-gray-300 shadow-lg p-4 rounded-xl hover:shadow-xl transition-all duration-300 flex flex-col justify-between h-[400px] group">
            {/* Product Image */}
            ...
            {/* Product Details */}
            <div className="mt-1 space-y-1 flex-col overflow-hidden">
                <Link to={`/product/${product._id}`}>
                    <h2 className="text-lg font-bold text-gray-800 hover:text-primary transition-colors duration-300 line-clamp-2">
                        {product.name}
                    </h2>
                </Link>
                <div className="flex items-center justify-between">
                    <p className="text-gray-500 text-sm">
                        {isBrand(product.brand) ? product.brand.name : product.brand}
                    </p>
            ...
            <div className="mt-4">
                <Button
                    ...
                    className="w-full bg-primary-foreground text-white py-2 hover:bg-primary rounded-lg disabled:opacity-60 disabled:pointer-events-none disabled:cursor-not-allowed"
                >
